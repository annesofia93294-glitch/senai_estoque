...
def init_db():
    ...
    # Tabela com suporte a empréstimo entre unidades (unidade original vs unidade atual)
    cursor.execute("""
...
# CACHE OTIMIZADO PARA VELOCIDADE MÁXIMA (5 minutos de TTL)
...
def upload_imgbb(imagem_upload):
    try:
        ...
        with st.spinner("Otimizando e enviando imagem para o servidor seguro..."):
...
# --- CABEÇALHO COM LOGOTIPO DO SENAI ---
col_logo, col_titulo = st.columns([1, 4])
with col_logo:
    # URL pública estável da logo do SENAI para o cabeçalho
    st.image("https://upload.wikimedia.org/wikipedia/commons/8/8e/Logo_SENAI.svg", width=150)
with col_titulo:
...
if menu == "Visualizar Estoque":
    ...
    if not df_estoque.empty:
        ...
        for _, row in df_estoque.iterrows():
            ...
            # Destaca se o item está emprestado de outra unidade
            if row["unidade_origem"] != row["unidade_atual"]:
                c_loc.markdown(f"📍 **{row['unidade_atual']}**<br><span style='color:orange; font-size:12px;'>Origem: {row['unidade_origem']}</span>", unsafe_allow_html=True)
            else:
                c_loc.write(row["unidade_atual"])
    ...
    if itens_disponiveis.empty:
        ...
        with col_info:
            ...
            if origem_atual != unidade_selecionada:
                st.warning(f"⚠️ Este material está emprestado e pertence originalmente a **{origem_atual}**.")
        ...
        if acao == "Dar Baixa (Saída Definitiva)":
            ...
            if st.button("Confirmar Baída/Saída"):
        ...
        # 2. EMPRÉSTIMO ENTRE UNIDADES (RESOLVE O PROBLEMA DE LUIS EDUARDO / BARREIRAS)
            ...
            if st.button("Confirmar Envio / Empréstimo"):
                ...
                # Se for enviar tudo ou parte
                if qtd_envio == qtd_atual:
                    # Apenas muda a unidade atual no banco, preservando a unidade de origem!
                    execute_db("UPDATE estoque_pro SET unidade_atual = %s WHERE id = %s", (nova_unidade_atual, item_id_selecionado))
                else:
                    # Subtrai da unidade atual e cria um novo registro na unidade de destino mantendo a origem original
                    ...
                    # Verifica se a unidade destino já tem esse item
        ...
        # 3. EDIÇÃO PURA (SEM REGISTRAR NO HISTÓRICO DE SAÍDAS)
            ...
            if st.button("Salvar Alterações Cadastrais"):
                ...
                st.success("Alterações salvas com sucesso (sem gerar falsas saídas no histórico)!")
                st.rerun()
